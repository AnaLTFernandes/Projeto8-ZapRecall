...
export default function Flashcard({
	...
	const className = `flashcard ${cardStatus} ${cardResult} ${
		other.secret ? "secret" : ""
	}`.trim();
	...
	function setStates(classResult, iconName) {
		...
		setIconsResult([
			...iconsResult,
			{ iconName: iconName, iconClass: classResult },
		]);
	...
	if (cardStatus === "") {
		template = (
			<div key={index} className={className}>
				...
				<ion-icon
					name="play-outline"
					onClick={() => setCardStatus("progress-ask")}
				></ion-icon>
			</div>
		);
		...
		template = (
			<div key={index} className={className}>
				...
				<img
					alt="turn"
					src={arrowIMG}
					onClick={() => setCardStatus("progress-answer")}
				/>
			</div>
		);
